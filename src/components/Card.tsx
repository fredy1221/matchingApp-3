...
export function Card({ user, onLike, onPass, onHobbyClick }: CardProps) {
  ...
  const handleHobbyTagClick = (hobby: string) => {
    console.log(`Navigating to /hobby/${hobby}`);
    navigate(`/hobby/${hobby}`);
  };

  const handleViewProfileClick = () => {
    const profilePath = `/profile/${user.id}`;
    console.log(`Clicked View Profile button, navigating to ${profilePath}`);
          ...
          <div className="text-white">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {user.hobbies.map((hobby) => (
                <CustomButton
                  key={hobby}
                  logMessage={`Clicked on hobby tag: ${hobby}`}
                  onClick={() => handleHobbyTagClick(hobby)}
